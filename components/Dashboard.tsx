...
const Dashboard: React.FC<DashboardProps> = ({ transactions, onEditTransaction, isDarkMode, toggleTheme }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto h-full flex flex-col p-6 transition-colors duration-300 font-sans">
      
      {/* Header - Centered Logo */}
      <div className="relative flex items-center justify-center mb-6 pt-4 animate-fade-in">
      ...
      <div className="flex bg-gray-200 dark:bg-[#1E1E1E] p-1 rounded-2xl mb-8 self-center w-full max-w-xs animate-slide-up" style={{ animationDelay: '0.1s' }}>
         <button 
            ...
            className={`flex-1 py-2 text-sm font-semibold rounded-xl transition-all duration-300 ${viewMode === 'recent' ? 'bg-white dark:bg-[#333] shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
         >
            Resumen
         </button>
         <button 
            onClick={() => setViewMode('history')}
            className={`flex-1 py-2 text-sm font-semibold rounded-xl transition-all duration-300 ${viewMode === 'history' ? 'bg-white dark:bg-[#333] shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
         >
            Historial
         </button>
      </div>
      ...
      {viewMode === 'recent' ? (
        <>
            ...
            <div className="mb-10 text-center animate-slide-up" style={{ animationDelay: '0.2s' }}>
                ...
                <div className="flex justify-center gap-4 mt-8">
                    <div className="flex items-center gap-2 bg-white dark:bg-[#1E1E1E] px-5 py-3 rounded-2xl shadow-sm border border-gray-100 dark:border-[#333] transition-transform hover:scale-105">
            ...
            {/* Transactions List */}
            <div className="flex-1 overflow-y-auto pr-2 pb-24 animate-slide-up" style={{ animationDelay: '0.3s' }}>
                <div className="flex justify-between items-center mb-4 sticky top-0 bg-[#F5F5F5] dark:bg-[#121212] py-2 z-10 transition-colors duration-300">
                ...
                <div className="space-y-3">
                    {transactions.map((t) => (
                        ...
                        className="group bg-white dark:bg-[#1E1E1E] p-4 rounded-2xl shadow-sm border border-transparent hover:border-indigo-100 dark:hover:border-indigo-900/50 cursor-pointer transition-all flex items-center justify-between hover:translate-x-1"
                        >
        ...
        <div className="flex-1 overflow-y-auto pr-2 pb-24 animate-slide-up" style={{ animationDelay: '0.2s' }}>
             <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-6 sticky top-0 bg-[#F5F5F5] dark:bg-[#121212] py-2 z-10 transition-colors duration-300">Balance Mensual</h2>
                                        ...
                                        <div className="space-y-3">
                                            {data.transactions.map((t) => (
                                                <div 
                                                    ...
                                                    className="bg-white dark:bg-[#252525] p-3 rounded-xl flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2C2C2C] transition-colors"
                                                >
                                                    <div className="flex items-center gap-3">
